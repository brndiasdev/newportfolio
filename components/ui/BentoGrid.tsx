...
export const BentoGridItem = ({
	...
	const handleCopy = () => {
		navigator.clipboard.writeText("brndiasdev@gmail.com");
		...
		>
			<div className={`${id === 6 && "flex justify-center"} h-full`}>
				...
				<div className={cn(titleClassName, "group-hover/bento:translate-x-2 transition duration-200 relative md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10")}>
					<div className='font-sans font-extralight md:max-w-40 md:text-xs lg:text-base text-sm text-[#C1C2D3] z-10'>{description}</div>
					...
					{id === 3 && (
						<div className='flex  gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2'>
							...
							<div className='flex flex-col gap-3 lg:gap-8'>
								<span className='py-4 px-3 rounded-lg text-center bg-[#10132e]' />
								{["Umi.js", "Angular", "MongoDB"].map((item) => (
									<span key={item} className='py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10123E]'>
										{item}
									</span>
								))}
